...
`Javascript` specialist, expert in `node.js` and `React`.
Commander of `git`, `linux`, `docker` and all things software.
Backend work preferred. Can also write `Golang`.
Passionate about _beautiful_, _readable_, _scalable_ code.
Hire me to solve your problems!
...
* Worked in a team varying from 3 to 12 people
* Initially hired as senior backend `Typescript` developer, later transitioned to `Go` developer
...
* Brought up and maintained `AWS` services such as `RDS`, `DynamoDB`, `S3` and related network infrastructure
* Created a service to supply OTA firmware updates to bikes
* Created a service to track and investigate manufacturing faults
* Maintained & improved some smaller ERP-related internal tools
* Proactively found and fixed accounting synchronisation errors
...
> _Project name cannot be revealed_
